...
module.exports = function login () {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    var password = security.hash(req.body.password) || ''
    var query = 'SELECT * FROM Users WHERE email = ? AND password = ? AND deletedAt IS NULL'
   // models.sequelize.query(`SELECT * FROM Users WHERE email = '${req.body.email || ''}' AND password = '${security.hash(req.body.password || '')}' AND deletedAt IS NULL`, { model: UserModel, plain: true }) // vuln-code-snippet vuln-line loginAdminChallenge loginBenderChallenge loginJimChallenge
    models.sequelize.query(query, { replacements: [email, password], model: UserModel, plain: true })   
